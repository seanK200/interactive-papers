...
function processAnnotations() {
  ...
  for (let i = 0; i < annotations.length; i++) {
    ...
    if (
      annotations[i].getAttribute("href") === null ||
      annotations[i].getAttribute("href") === ""
    ) {
      ...
      if (annotations[i].getAttribute("data-ip-footnote-id") === null) {
        annotations[i].setAttribute(
          "data-ip-footnote-id",
          annotations[i].innerText
        );
      }
    }
  }
...
function processFootnotes() {
  let footnotes = document
    .getElementById("footnotes-div")
    .getElementsByClassName("footnote");
  for (let i = 0; i < footnotes.length; i++) {
    const footnoteIdDiv = document.createElement("div");
    if (!footnotes[i].classList.contains("hide-id")) {
      const footnoteIdContent = document.createTextNode(
        footnotes[i].getAttribute("id")
      );
...
function handleFootnoteClick(footnoteElement) {
  ...
  if (footnoteId === null || footnoteId === "") {
    ...
    alert(
      "[ERROR] A 'data-ip-footnote-id' attribute of a footnote link(<a>) tag cannot be left empty. Remove the attribute entirely if you are not using it."
    );
  } else {
...
function getFootnotesById(footnoteId, returnIndexes = false) {
  ...
  for (let i = 0; i < footnotes.length; i++) {
    if (footnotes[i].getAttribute("data-ip-footnote-id") === footnoteId) {
      footnotes[i].setAttribute(
        "data-ip-same-footnote-index",
  ...
  return returnIndexes
    ? [foundFootnotes, foundFootnoteIndexes]
    : foundFootnotes;
}
...
function setFootnoteActionsMultipleDiv(selectedFootnoteElement) {
  const footnoteId =
    selectedFootnoteElement.getAttribute("data-ip-footnote-id") !== null
      ? selectedFootnoteElement.getAttribute("data-ip-footnote-id")
      : selectedFootnoteElement.getAttribute("id");
  const [foundFootnotes, foundFootnoteIndexes] = getFootnotesById(
    footnoteId,
    true
  );
  if (foundFootnotes !== null) {
    if (foundFootnotes.length > 1) {
      let current_same_index = selectedFootnoteElement.getAttribute(
        "data-ip-same-footnote-index"
      );
      if (current_same_index !== null && current_same_index !== "null")
        current_same_index = parseInt(current_same_index);
      const prev_same_index =
        current_same_index > 0 ? current_same_index - 1 : null;
      const prev_index =
        prev_same_index === null ? null : foundFootnoteIndexes[prev_same_index];
      const next_same_index =
        current_same_index < foundFootnotes.length - 1
          ? current_same_index + 1
          : null;
      const next_index =
        next_same_index === null ? null : foundFootnoteIndexes[next_same_index];
      // console.log(current_same_index, prev_same_index, prev_index, next_same_index, next_index);

      // set prev btn attributes
      document
        .querySelector("div#footnote-actions-multiple #prev-btn")
        .setAttribute("data-ip-prev-footnote-index", prev_index);

      // set current span
      document.querySelector(
        "div#footnote-actions-multiple span#current"
      ).innerHTML = parseInt(current_same_index) + 1;

      // total span
      document.querySelector(
        "div#footnote-actions-multiple span#total"
      ).innerHTML = foundFootnotes.length;

      // next btn
      document
        .querySelector("div#footnote-actions-multiple #next-btn")
        .setAttribute("data-ip-next-footnote-index", next_index);

      document.querySelector("div#footnote-actions-multiple").style.display =
        "none";
    } else {
      document.querySelector("div#footnote-actions-multiple").style.display =
        "none";
...
function scrollFootnoteIntoView(footnoteElement) {
  let location =
    footnoteElement.offsetTop -
    document.getElementById("content-div").offsetTop -
